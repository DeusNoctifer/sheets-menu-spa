...
// Definimos los cupos gratuitos por categoría
const FREE_LIMITS = {
  PROTEIN: 1,
  TOPPING: 1,
  SAUCE: 3
...
export function BuilderPage() {
  ...
  const renderSection = (title: string, category: string, step: number) => {
    ...
    // --- NUEVO: ¿Esta sección ya tiene algo seleccionado? ---
    const hasSelection = 
    ...
    return (
      <div className={`rounded-2xl border transition-all overflow-hidden ${
        // Lógica visual para la tarjeta contenedora según estado
        ...
        >
          <div className="flex items-center gap-2">
            <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-black shadow-sm transition-colors ${
              // El círculo del número se pinta si está abierto o si ya se seleccionó algo
              isOpen || hasSelection ? 'bg-brand-primary text-white' : 'bg-gray-100 text-brand-text/50'
            }`}>
              {step}
            </span>
            <div className="flex flex-col items-start">
              <h3 className={`font-black uppercase tracking-tight text-sm transition-colors ${
                // El título se pinta si está abierto o si ya se seleccionó algo
                isOpen || hasSelection ? 'text-brand-primary' : 'text-brand-text'
              }`}>{title}</h3>
              {!isOpen && (
                <span className={`text-[11px] font-bold -mt-0.5 transition-colors ${
                  // El texto de resumen también cambia de color
          ...
          {isOpen ? <ChevronUp size={20} className="text-brand-primary" strokeWidth={3} /> : 
           hasSelection ? <Check size={20} className="text-brand-primary" strokeWidth={3} /> : 
              ...
              return (
                ...
                >
                  <div className="flex flex-col items-start text-left">
                    <span className={`font-bold text-sm ${isSelected ? 'text-brand-primary' : 'text-brand-text'}`}>
                      {ing.name}
                    </span>
                    {showPrice && (
  ...
  return (
    // --- pb-40 para dar espacio a los dos footers apilados ---
    <div className="pb-40">
      ...
      {/* --- bottom-16 para que quede encima del menú global --- */}
